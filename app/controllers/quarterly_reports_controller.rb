class QuarterlyReportsController < ApplicationController
  ...
  def submit_form
    ...
    selected_quarter = params[:quarter_select]
    selected_year = params[:date]['year'].to_s.to_i
    
    if selected_quarter != nil
      start_month = selected_quarter.to_s.split('-')[0].to_i
      end_month = selected_quarter.to_s.split('-')[1].to_i
      start_date = Date.new(selected_year, start_month, 1)
      end_date = Date.new(selected_year, end_month, -1)
    ...
    if !state.empty? && !start_date.empty? && !end_date.empty?
      ...
      if start_date_in_date <= end_date_in_date
        redirect_to action: "show_qpr",state: state, district: district, cell: cell, start_date: start_date, end_date: end_date
      end
    end
  ...
  def show_qpr
    ...
    end_month = Date.parse(end_date).strftime("%m")
