class OverallPsychosocialCasesController < ApplicationController
	def index
	...
	def redirect_to_index
		redirect_to '/all_reports/overall_psychosocial_cases'
	end
    ...
    for year in year_array
			...
			for i in trial_court_cases
				if i['key'][0]!=nil
					if i['key'][0].split("-")[0].to_i == year	
						if i['key'][1][0]!=nil
							if i['key'][1][0].include? "psy"
								@psycho_cases += 1
							end
						end
						if i['key'][1][0]!=nil	
							if i['key'][1][0].include? "psy" and i['key'][1][1].include? "closed"
								@closed_cases += 1
							end
						end
					end
			...
			@remaining = @psycho_cases - @closed_cases
			@data.push({
				"year" => year,
				"psycho_cases" => @psycho_cases,
